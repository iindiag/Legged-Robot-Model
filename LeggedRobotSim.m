...
thetaGeneration = sineWave(theta_ref, t, f_hip, f_knee, phi_hip, phi_knee);

figure(1)
subplot(2, 2, 1)
plot(t, thetaGeneration(:,1))
hold on
plot(t, thetaGeneration(:,2))
xlabel('Time (s)')
ylabel('Angle (rad)')
legend('Hip Joint', 'Knee Joint')
title('Gait Generated Angle Offsets over Time (MATLAB)')

CartesianCoords = forwardKinematics(l_1, l_2, thetaGeneration);
...
knee_x = l_1*sin(thetaGeneration(:,1));
knee_z = -(l_1*cos(thetaGeneration(:,1)));
...
ylabel('Current (Amps)')
xlabel('Time (s)')
title('Current Generated by PD Controller (excplicit Euler)')
grid on
...
legend('Hip Joint','Knee Joint');
grid on;
title('Current Generated by PD Controller (ode45)');
...
subplot(2, 1, 1)
plot(t, C_zCoords)
title('z-Coordinate of Foot over Time (explicit Euler)')
xlabel('Time (s)')
ylabel('z Position (m)')
grid on
...
title('z-Coordinate of Foot over Time (ode45)')
xlabel('Time (s)')
ylabel('z Position (m)')
grid on
