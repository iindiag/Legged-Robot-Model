...
subplot(3,1,2)
plot(t, omegaKnee, 'Color', [0.5 0 0.5], 'LineWidth', 1.2)
xlabel('Time (s)')
...
subplot(2,1,1)
plot(t, abs(dLog), 'LineWidth', 1.5, 'Color', [0.62 0.72 0.54]);
xlabel('Time (s)')
ylabel('Distance (m)')
title('Distance Travelled over Time')
grid on

disp(['Total displacement: ', num2str(dNet), ' m'])

subplot(2,1,2)
plot(t, COT, 'LineWidth', 1.5, 'Color', [0.13 0.45 0.5]);
xlabel('Time (s)')
ylabel('Cost of Transport (-)')
title('Cost of Transport over Time')
grid on

%saveas(gcf, 'SineDistAndCOT.png')

% Computing the MC
MCVals = MCCalculation(B, H, C_zCoords, iKnee, omegaKnee);
MCSmooth = smoothdata(MCVals, 'gaussian', 5);

figure(5)
plot(t(1:end-1), MCVals, 'LineWidth', 1, 'Color', [0.0 0.0 0.5]);
xlabel('Time (s)');
ylabel('Morphological Computation (-)');
title('Morphological Computation over Time');
grid on;

%saveas(gcf, 'SineMC.png')