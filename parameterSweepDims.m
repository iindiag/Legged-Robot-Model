...
g = 9.81;
lTotal = db.dims.l_1 + db.dims.l_2;

upperLengths = 0.08:0.01:0.24;
lowerLengths = lTotal - upperLengths;

for i = 1:length(hipPitchKp)
    for j = 1:length(hipPitchKd)
        for m = 1:length(kneeKp)
            for n = 1:length(kneeKd)
                for k = 1:length(upperLengths)
                
                    % Loading defaults
                    db = database();

                    db.dims.l_1 = upperLengths(k);
                    db.dims.l_2 = lowerLengths(k);
...
resultsTable = array2table(results,...
    ...
    'Knee K_p','Knee K_d','Link Length 1', 'Link Length 2',...
